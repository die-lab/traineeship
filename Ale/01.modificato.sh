...
#se la specie analizzata ha pi genomi mitocondriali, ossia uno per sesso, decomenntare la riga seguente e commentare quella tre righe prima (per il mitocondriale).
#genome_mit_file=$organism"_mit"_$sex".fasta"

#si riprende i genomi mitocondriali e nucleari dalla cartella apposita in base alla specie settata. Commenta se li hai inseriti a mano.
cp /home/diegocarli/genome.fasta/$genome_mit_file .
cp /home/diegocarli/genome.fasta/$genome_nuc_file .
#cp /media/storage/stefanosquartecchia/Genomi/$genome_mit_file .
#cp /media/storage/stefanosquartecchia/Genomi/$genome_nuc_file .
...
#oppure commmenta entrambe e metti i file fastq a mano nella directory.
SRA_entries=`grep $organism"_"$sex /home/diegocarli/genome.fasta/SRA_tabella | awk '{print $2}' | sed 's/,/ /g'`
#SRA_entries="SRR6649427 SRR6649426"
...
cp /home/stefanosquartecchia/$adapter_file .
for fastq in *.fastq
...
cp 02.modificato.sh BedFiles
cp 03.modificato.sh BedFiles
cp *.R BedFiles
...
#blocco per modificare header ddi alcuni fastq, come quelli per RuPh
#in caso di fastq già formattate giuste non dovrebbe essere un problea. se lo è, commentale.
sed -i 's/_cds//g' $output_fa
sed -i 's/forward//' $output_fa
...
#MOD ALE
for cluster in `ls`; do num_sequences=`grep -c ">" $cluster`; for s in `grep ">" $cluster | sed 's/>//g' | sed 's/\([0-9]\+\)_\([0-9]\+\)_\([0-9]\+\)/\1:\2:\3/g'`; do grep -w $s ../*bed >> ../$clusters_bed/${cluster%"fa"}"bed"; done ;done
...
#MOD ALE
for grepfile in `grep ">" $output_centroids | sed 's/>//g'`; do s=`echo $grepfile | sed 's/\([0-9]\+\)_\([0-9]\+\)_\([0-9]\+\)/\1:\2:\3/g'`; check=`grep -w $s $clusters_bed/*.sorted.bed`;	if [ -n "$check" ]; then grep -w -A 1 ">"$grepfile $output_centroids >> $output_selected_centroids; fi; done
